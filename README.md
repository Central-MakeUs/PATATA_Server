...
**숨겨진 사진 스팟을 지도에서 발견하고 사용자 간 공유하는 위치 기반 커뮤니티 서비스**  
...
- `main` : 실제 배포용 브랜치 (운영 환경)
- `develop` : 개발 브랜치 (테스트 환경 및 통합 개발)
- `feature/기능명` : 새로운 기능 개발 시 사용  
  예: `feature/spotCRUD`, `feature/withdraw`

> 모든 기능 개발은 `feature/*` 브랜치에서 작업한 뒤 `develop` 브랜치로 Pull Request를 생성하고,  
> 코드 리뷰 및 테스트를 거쳐 병합합니다. 운영 배포 전에는 `develop → main` 으로 머지합니다.
...
[이미지 리사이징 구조 개선](https://velog.io/@pdy0207/SpringAWS-S3-S3-Lambda로-이미지-리사이징-구조-개선)
